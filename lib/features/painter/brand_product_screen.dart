...
///
/// This is the reusable template used by every dynamically-created brand. It is
/// a structural clone of the Birla Opus screen — identical layout, navigation
/// flow, screen hierarchy and product browsing experience — with only the
/// brand-specific DATA differing (brand name, cover image, colors, categories,
/// products), all of which is driven from [DataService] by the brand name.
///
/// Level 0: Main categories
/// Level 1: Sub-categories (only when a category has multiple sub-categories)
/// Level 2: Product listing for the selected category (+ sub-category)
///
/// Special case: a category literally named "Wall Putty" gets the same
/// putty-specific ordering flow as Birla Opus (packet / custom kg).
class BrandProductScreen extends ConsumerStatefulWidget {
  final String brand;
  const BrandProductScreen({super.key, required this.brand});

  @override
  ConsumerState<BrandProductScreen> createState() => _BrandProductScreenState();
}

class _BrandProductScreenState extends ConsumerState<BrandProductScreen> {
  // Navigation state
  String? _selectedCategory; // Level 0 selection
  String? _selectedSubCategory; // Level 1 selection
  bool _showPuttyOptions = false; // Putty level 1
  ProductModel? _selectedPuttyProduct; // Putty level 2 selection
  bool _showPuttyChoice = false; // Putty level 2 (Customize vs Fixed)
  bool _showPuttyPacket = false; // Putty level 3 (Fixed size selection)
  bool _showPuttyCustom = false; // Putty level 3 (Manual kg entry)

  final _customKgCtrl = TextEditingController();

  // UI metadata mapping for known category names. Unknown categories fall back
  // to a neutral icon/color, so any brand's categories render cleanly.
  ...
  String get _brand => widget.brand;
  Color get _brandColor => AppColors.getBrandPrimary(_brand);
  ...
  Widget _buildHeader() {
    ...
    final brandModel = ds.getBrandByName(_brand);
    final coverUrl = brandModel?.headerImageUrl;
    final gradient = AppColors.getBrandGradient(_brand);
        ...
        children: [
          ...
          if (_selectedCategory == null) ...[
            ...
            Padding(
              ...
              child: Text(
                'Browse products & place your order',
  ...
  Widget _getCurrentView() {
    ...
    if (_selectedCategory != null) {
      final ds = ref.watch(dataServiceProvider);
      final subCatGroups = ds.getProductsGroupedBySubCategory(_brand, _selectedCategory!);
      final subCats = subCatGroups.keys.where((s) => s.isNotEmpty && s != 'None').toList();

      // If there's only one sub-category or it's empty, go directly to products
  ...
  Widget _buildMainCategories({Key? key}) {
    ...
    final availableCategories = ds.getCategoriesForBrand(_brand).where((c) => c != 'None' && c.isNotEmpty).toList();
  ...
  Widget _categoryCard(String name, IconData icon, Color color) {
    final ds = ref.watch(dataServiceProvider);
    final productCount = ds.getProductsByCategory(_brand, name).length;
  ...
  Widget _buildProductListing({Key? key}) {
    ...
    if (_selectedSubCategory != null) {
      products = ds.getProductsByCategory(_brand, category).where(
        (p) => p.subCategory.toLowerCase() == _selectedSubCategory!.toLowerCase(),
      ).toList();
    } else {
      products = ds.getProductsByCategory(_brand, category);
    }
  ...
  Widget _buildPuttyProductSelection({Key? key}) {
    final ds = ref.watch(dataServiceProvider);
    final puttyProducts = ds.getProductsByCategory(_brand, 'Wall Putty');
