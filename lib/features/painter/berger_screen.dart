...
/// Berger multi-level product selection screen.
///
/// Level 0: Main categories (e.g., Interior, Exterior, Enamels)
/// Level 1: Sub-categories (e.g., Luxury Emulsion, Premium) 
/// Level 2: Product listing for the selected category + sub-category
class BergerScreen extends ConsumerStatefulWidget {
  const BergerScreen({super.key});

  @override
  ConsumerState<BergerScreen> createState() => _BergerScreenState();
}

class _BergerScreenState extends ConsumerState<BergerScreen> {
  // Navigation state
  ...
  final Color _brandColor = AppColors.getBrandPrimary('Berger');
            ...
            child: Column(
              children: [
                ...
                Builder(builder: (ctx) {
                  ...
                  final offers = ds.getActivePromotions().where((p) => p.brand.toLowerCase() == 'berger').toList();
                  return BrandOffersWidget(offers: offers, brand: 'Berger');
                }),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          ...
          colors: [AppColors.bergerGradientStart, AppColors.bergerGradientEnd],
        ),
        boxShadow: [
          BoxShadow(
          ...
          BoxShadow(
            color: AppColors.bergerGradientEnd.withValues(alpha: 0.5),
            blurRadius: 14,
            spreadRadius: 1,
  ...
  Widget _getCurrentView() {
    ...
    if (_selectedCategory != null) {
      final ds = ref.watch(dataServiceProvider);
      final subCatGroups = ds.getProductsGroupedBySubCategory('Berger', _selectedCategory!);
  ...
  Widget _buildMainCategories({Key? key}) {
    ...
    final availableCategories = ds.getCategoriesForBrand('Berger').where((c) => c != 'None' && c.isNotEmpty).toList();
  ...
  Widget _categoryCard(String name, IconData icon, Color color) {
    final ds = ref.watch(dataServiceProvider);
    final productCount = ds.getProductsByCategory('Berger', name).length;
  ...
  Widget _buildProductListing({Key? key}) {
    ...
    if (_selectedSubCategory != null) {
      products = ds.getProductsByCategory('Berger', category).where(
        (p) => p.subCategory.toLowerCase() == _selectedSubCategory!.toLowerCase(),
      ).toList();
    } else {
      products = ds.getProductsByCategory('Berger', category);
    }
