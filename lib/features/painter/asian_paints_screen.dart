...
class AsianPaintsScreen extends ConsumerStatefulWidget {
  const AsianPaintsScreen({super.key});

  @override
  ConsumerState<AsianPaintsScreen> createState() => _AsianPaintsScreenState();
}

class _AsianPaintsScreenState extends ConsumerState<AsianPaintsScreen> {
  // Navigation state
  String? _selectedCategory; // Level 0 selection
  String? _selectedSubCategory; // Level 1 selection

  // UI Metadata mapping for known categories (Fallback gracefully if not mapped)
  static final Map<String, Map<String, dynamic>> _categoryUIMetadata = {
    'Interior Paints': {'icon': Icons.weekend_rounded, 'color': const Color(0xFF6366F1)},
    'Exterior Paints': {'icon': Icons.house_rounded, 'color': const Color(0xFF10B981)},
    'Enamels': {'icon': Icons.format_paint_rounded, 'color': const Color(0xFFEF4444)},
    'Primers': {'icon': Icons.brush_rounded, 'color': const Color(0xFFEAB308)},
    'Waterproofing': {'icon': Icons.water_drop_rounded, 'color': const Color(0xFF3B82F6)},
    'Wood Finishes': {'icon': Icons.park_rounded, 'color': const Color(0xFFA16207)},
    'Undercoats': {'icon': Icons.layers_rounded, 'color': const Color(0xFF8B5CF6)},
  };

  final Color _brandColor = AppColors.getBrandPrimary('Asian Paints');
            ...
            child: Column(
              children: [
                ...
                Builder(builder: (ctx) {
                  ...
                  final offers = ds.getActivePromotions().where((p) => p.brand.toLowerCase() == 'asian paints').toList();
                  return BrandOffersWidget(offers: offers, brand: 'Asian Paints');
                }),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          ...
          colors: [AppColors.asianPaintsGradientStart, AppColors.asianPaintsGradientEnd],
        ),
        boxShadow: [
          BoxShadow(
          ...
          BoxShadow(
            color: AppColors.asianPaintsGradientEnd.withValues(alpha: 0.5),
            blurRadius: 14,
            spreadRadius: 1,
      ...
      child: Column(
        children: [
          ...
          if (_selectedCategory == null) ...[
            ...
            Center(
              child: Image.network(
                'https://mlzrqgocvenrwjnabljm.supabase.co/storage/v1/object/public/paint-images/brands/ap_big.png',
                height: 90,
                fit: BoxFit.contain,
              ),
            ),
  ...
  Widget _getCurrentView() {
    ...
    if (_selectedCategory != null) {
      final ds = ref.watch(dataServiceProvider);
      final subCatGroups = ds.getProductsGroupedBySubCategory('Asian Paints', _selectedCategory!);
  ...
  Widget _buildMainCategories({Key? key}) {
    ...
    final availableCategories = ds.getCategoriesForBrand('Asian Paints').where((c) => c != 'None' && c.isNotEmpty).toList();
  ...
  Widget _categoryCard(String name, IconData icon, Color color) {
    final ds = ref.watch(dataServiceProvider);
    final productCount = ds.getProductsByCategory('Asian Paints', name).length;
  ...
  Widget _buildProductListing({Key? key}) {
    ...
    if (_selectedSubCategory != null) {
      products = ds.getProductsByCategory('Asian Paints', category).where(
        (p) => p.subCategory.toLowerCase() == _selectedSubCategory!.toLowerCase(),
      ).toList();
    } else {
      products = ds.getProductsByCategory('Asian Paints', category);
    }
