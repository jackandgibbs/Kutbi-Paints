...
b.createdAt.compareTo(a.createdAt));
  ...
  Future<bool> checkAndAutoResetPoints() async {
    try {
      final now = DateTime.now();
      final isLastDayOfMonth = now.day == DateTime(now.year, now.month + 1, 0).day;
      
      if (!isLastDayOfMonth) return false;

      // Check if already reset today
      final todayKey = '${now.year}-${now.month}-${now.day}';
      final lastResetData = await _sb.from('app_settings').select().eq('key', 'last_auto_reset').maybeSingle();
      
      if (lastResetData != null && lastResetData['value'] == todayKey) {
      ...
      if (allPainters.isEmpty) return false;
      ...
      // Overwrite same-month record if it exists.
      final existingIdx = _pointsHistory.indexWhere(
        (r) => r is Map && r['month'] == monthYear,
      );
      final recordId = existingIdx != -1
      ...
      await _sb.from('app_settings').upsert({'key': 'last_auto_reset', 'value': todayKey});

      notifyListeners();
      return true;
    } catch (e) {
      debugPrint('Error in auto reset: $e');
      return false;
    }
  }
